...
public class DaoComentario {
	...
	public ArrayList <Comentario> listarComentariosPorIsbn(long id, int tipo) throws SQLException{
		...
		if (con!=null) {
			...
			String sql = "SELECT c.ID, c.ISBN, c.Fecha_comentario, c.Texto, c.Titulo, v.valor, u.nombre FROM symposium.comentarios c, symposium.valoraciones v, symposium.usuarios u WHERE v.ID=c.ID and c.ID=u.ID and c.ISBN=? and u.ID in (SELECT ID FROM symposium.usuarios WHERE Nivel=?);";
			PreparedStatement ps = con.prepareStatement(sql);
			ps.setLong(1, id);
			ps.setInt(2, tipo);
